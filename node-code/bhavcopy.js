...
let fromStr = values.from || positionals || today;
let toStr = values.to || fromStr;
        ...
        Papa.parse(csvText, {
          ...
          step: function (results) {
            ...
            if (row.SERIES === 'EQ' && parseFloat(row.TTL_TRD_VAL || 0) > 10000000) {
              ...
              // e.g., await db.collection('bhavcopies').insertOne(stockRecord);
              console.log(`   [SAVE] ${stockRecord.symbol} -> Close: ${stockRecord.close}`);
            }
          },
          complete: function () {
            console.log(`✅ Completed parsing for ${dateFormatted}`);
            resolve();
          }
        });
      });
...
// Execute pipeline
await processBhavcopyRange(fromStr, toStr);