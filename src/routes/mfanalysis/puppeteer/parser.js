...
export async function parseZip(file) {
    ...
    const excelFiles = Object.values(zip.files).filter((entry) => {
    ...
    if (excelFiles.length === 0) {
        throw new Error('No XLSX or XLS files found inside the ZIP.');
    }

    if (excelFiles.length === 1) {
        const buffer = await excelFiles[0].async('arraybuffer');
        return buffer;
    }
    ...
    for (const excelFile of excelFiles) {
        ...
        for (const sheetName of workbook.SheetNames) {
            ...
            const baseName = `${excelFile.name}:${sheetName}`
                .replace(/[\\/*?:[\]]/g, '_')
                .substring(0, 31);
            ...
            while (combinedWorkbook.SheetNames.includes(finalName)) {
    ...
    const output = XLSX.write(combinedWorkbook, {
        bookType: 'xlsx',
        type: 'array'
    });
