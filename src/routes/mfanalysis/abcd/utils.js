 ...
 *
 * @param {Array<{time: date, value:number}>} lineData
 * @param {"W"|"M"} interval
 * @returns {Array<{time:number, open:number, high:number, low:number, close:number}>}
 */
export function LineDataToBarData(lineData, interval) {

    if (interval !== 'W' && interval !== 'M') {
        throw new Error(`Unsupported interval: ${interval}`);
    }

    lineData = lineData.map(item => ({ timestamp: new Date(item.time).getTime(), value: item.value }));
